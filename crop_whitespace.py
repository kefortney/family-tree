...
Usage:
    ...
    python crop_whitespace.py --threshold 240
...
def get_bounding_box(img, threshold=240):
    ...
    for y in range(height):
        for x in range(width):
            px = pixels[x, y]
            r, g, b = px[:3] if len(px) >= 3 else px
...
def crop_image(image_path, threshold=240, backup=False):
    ...
    try:
        ...
        if bbox is None:
            return (False, "Image is entirely white (skipped)")
        ...
        if new_width == original_size[0] and new_height == original_size[1]:
            return (False, "No whitespace to crop (same size)")
        ...
        reduction_pct = (
            100 * (1 - (new_width * new_height) / (orig_w * orig_h))
        )
        return (
            True,
            f"Cropped from {orig_w}\u00d7{orig_h} to "
            f"{new_width}\u00d7{new_height} ({reduction_pct:.1f}% smaller)",
        )
...
Examples:
  ...
  python crop_whitespace.py --backup
    ...
    for i, img_path in enumerate(image_files, 1):
        ...
        success, message = crop_image(
            str(img_path), args.threshold, args.backup
        )
